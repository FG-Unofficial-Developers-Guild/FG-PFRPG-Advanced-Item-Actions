...
function addToSpellDB(nodeItem)
...
function removeFromSpellDB(nodeItem)
	if not nodeItem then
		return;
	end
...
function addToWeaponDB(nodeItem)
	...
	if CharManager.hasFeat(nodeChar, "Two-Weapon Fighting") then
	...
	if bMelee then
		...
		if nodeWeapon then
			...
			addDamageToWeapon(nodeWeapon, aDamage, nBonus, "strength", nStatMult, aCritMult, sFinalDamageType1, bIsCorrosive, bIsFlaming, bIsFrost, bIsShocking);
		end
	end
	...
	if bMelee and bDouble then
		...
		if nodeWeapon then
			...
			addDamageToWeapon(nodeWeapon, aDamage, nBonus, "strength", 1, aCritMult, sFinalDamageType1, bIsCorrosive, bIsFlaming, bIsFrost, bIsShocking);
		end
	end
	...
	if bMelee and bDouble then
		...
		if nodeWeapon then
			...
			addDamageToWeapon(nodeWeapon, aDamage, nBonus, "strength", 0.5, aCritMult, sFinalDamageType2, bIsCorrosive, bIsFlaming, bIsFrost, bIsShocking);
		end
	end
...
function addDamageToWeapon(nodeWeapon, aDamage, nBonus, sStat, nStatMult, aCritMult, sFinalDamageType, bIsCorrosive, bIsFlaming, bIsFrost, bIsShocking)
	...
	if nodeDmgList then
		...
		if bIsCorrosive then
			addWeaponDamage(nodeDmgList, aSpecialDmg, 0, "", 1, {0}, "acid");
		end
		if bIsFlaming then
			addWeaponDamage(nodeDmgList, aSpecialDmg, 0, "", 1, {0}, "fire");
		end
		if bIsFrost then
			addWeaponDamage(nodeDmgList, aSpecialDmg, 0, "", 1, {0}, "cold");
		end
		if bIsShocking then
			addWeaponDamage(nodeDmgList, aSpecialDmg, 0, "", 1, {0}, "electricity");
		end
	end
end

function addWeaponDamage(nodeDmgList, aDamage, nBonus, sStat, nStatMult, aCritMult, sFinalDamageType)
	if not nodeDmgList then
		return;
	end
...
function getSpecialProperties(sPropsLower)
