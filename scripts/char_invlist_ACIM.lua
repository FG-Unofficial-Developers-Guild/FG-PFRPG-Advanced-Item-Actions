...
local function usingExt(sExt)
	return StringManager.contains(Extension.getExtensions(), sExt)
end

function getSpellSet(nodeChar, sItemSource)
	if not nodeChar or sItemSource == "" then
		return
	end
...
local function getLoadedModules(tLoadedModules)
	...
	for _, sModuleName in ipairs(tAllModules) do
		...
		if tModuleData.loaded then
			tLoadedModules[tModuleData.name] = tModuleData.name
		end
	end
end
...
local function findSpellNode(sSpellName)
	...
	for _, sModuleName in pairs(tLoadedModules) do
		nodeSpell =
			findSpellInModule(DB.findNode("reference.spells" .. "@" .. sModuleName), trim_spell_name(sSpellName))
		if nodeSpell then
			break
		end
	end
...
local function onItemChanged(nodeField)
	...
	if not ActorManager.resolveActor(nodeChar) then
		return
	end
			...
			if nodeAction then
				for _, v in pairs(nodeAction) do
					if DB.getValue(v, "type") == "cast" then
						...
						end -- luacheck: globals SpellManager.addTags
...
local function getCasterLevelByClass(sClassName, sSpellClassLevel)
	...
	elseif
		StringManager.contains({ CLASS_NAME_ARCHANIST, CLASS_NAME_ORACLE, CLASS_NAME_SORCERER }, sLowerClassName)
	then
		return tonumber(sSpellClassLevel) * 2
	elseif
		StringManager.contains(
			{ CLASS_NAME_CLERIC, CLASS_NAME_DRUID, CLASS_NAME_SHAMAN, CLASS_NAME_WITCH, CLASS_NAME_WIZARD },
			sLowerClassName
		)
	then
		...
		if nUsesAvailable == 0 then
			if usingExt("FG-PFRPG-Enhanced-Items") and not bAnnnounced then
				ChatManager.SystemMessage(
					string.format(
						"%s has no remaining charges listed. Please check it for accuracy.",
						DB.getValue(nodeItem, "name", "This wand")
					)
				)
				bAnnnounced = true
			end
...
local function updateUsesRemaining(nodeTrigger, nodeItem, nodeSpellSet, nSpellLevel, bWand)
	local function writeUses(fieldName)
		...
		if not (nodeTrigger and DB.getPath(nodeTrigger):match(".+%.inventorylist%..+%." .. fieldName)) then
			...
			local nUsesRemaining = DB.getValue(nodeSpellSet, "availablelevel" .. nSpellLevel, 0)
				- DB.getValue(nodeSpellLevel, "totalcast", 0)
...
function inventoryChanged(nodeChar, nodeItem, nodeTrigger)
	if not (nodeChar and nodeItem) then
		return
	end
	...
	if not (bPotion or bWand or bScroll) then
		return
	end
...
function onInit()
	...
	DB.addHandler("charsheet.*.inventorylist.*.charge", "onUpdate", onItemChanged)
	if usingExt("Advanced Charsheet") then
		_sSpellset = "itemspellset"
	end
end
